...
options {
	...
	superClass = SomeParserBase;
}

source: statementList EOF;
...
// declaration | labeledStmt | goStmt | returnStmt | breakStmt | continueStmt | gotoStmt |
...
expression:
	...
	| unary_op = (
		PLUS
		| MINUS
		// | EXCLAMATION | CARET | STAR | AMPERSAND | RECEIVE
	) expression
	| expression mul_op = (
		STAR
		| DIV
		// | MOD | LSHIFT | RSHIFT | AMPERSAND | BIT_CLEAR
	) expression
...
// | expression LOGICAL_AND expression | expression LOGICAL_OR expression;

primaryExpr: operand;
// | conversion | methodExpr | primaryExpr ( (DOT IDENTIFIER) | index | slice_ | typeAssertion |
// arguments )
...
integer: DECIMAL_LIT;
// | BINARY_LIT | OCTAL_LIT | HEX_LIT | IMAGINARY_LIT | RUNE_LIT;
...
/*
 ...
 sourceFile: packageClause eos (importDecl eos)* ( (functionDecl | methodDecl | declaration) eos )*
 EOF;
 ...
 statement: declaration | labeledStmt | simpleStmt | goStmt | returnStmt | breakStmt | continueStmt
 | gotoStmt | fallthroughStmt | block | ifStmt | switchStmt | selectStmt | forStmt | deferStmt;
 ...
 simpleStmt: sendStmt | incDecStmt | assignment | expressionStmt | shortVarDecl;
 ...
 assign_op: ( PLUS | MINUS | OR | CARET | STAR | DIV | MOD | LSHIFT | RSHIFT | AMPERSAND | BIT_CLEAR
 )? ASSIGN;
 ...
 ifStmt: IF ( expression | eos expression | simpleStmt eos expression ) block ( ELSE (ifStmt |
 block) )?;
 
 switchStmt: exprSwitchStmt | typeSwitchStmt;
 
 exprSwitchStmt: SWITCH (expression? | simpleStmt? eos expression? ) L_CURLY exprCaseClause*
 R_CURLY;
 ...
 typeSwitchStmt: SWITCH ( typeSwitchGuard | eos typeSwitchGuard | simpleStmt eos typeSwitchGuard)
 ...
 // It's possible to replace `type` with more restricted typeLit list and also pay attention to nil
 maps mapType: MAP L_BRACKET type_ R_BRACKET elementType;
 ...
 expression: primaryExpr | unary_op = ( PLUS | MINUS | EXCLAMATION | CARET | STAR | AMPERSAND |
 RECEIVE ) expression | expression mul_op = ( STAR | DIV | MOD | LSHIFT | RSHIFT | AMPERSAND |
 BIT_CLEAR ) expression | expression add_op = (PLUS | MINUS | OR | CARET) expression | expression
 rel_op = ( EQUALS | NOT_EQUALS | LESS | LESS_OR_EQUALS | GREATER | GREATER_OR_EQUALS ) expression |
 expression LOGICAL_AND expression | expression LOGICAL_OR expression;
 

 
 primaryExpr: operand | conversion | methodExpr | primaryExpr ( (DOT IDENTIFIER) | index | slice_ |
 ...
 literalType: structType | arrayType | L_BRACKET ELLIPSIS R_BRACKET elementType | sliceType |
 mapType | typeName;
 ...
 slice_: L_BRACKET ( expression? COLON expression? | expression? COLON expression COLON expression )
 R_BRACKET;
 
 typeAssertion: DOT L_PAREN type_ R_PAREN;
 
 arguments: L_PAREN ( (expressionList | nonNamedType (COMMA expressionList)?) ELLIPSIS? COMMA? )?
 R_PAREN;
